...
class avoidLeftArith{
    ...
    constructor(){
        this.textOutput="";
        this.counter = eoLR.counter;
        this.avoidLeftRecursorArith = eoLR.avoidLeftRecursorArith;
        this.avoidLeftRecursorVeriable = eoLR.avoidLeftRecursorVeriable;//消除左递归用到的全局变量

        this.phaseIndex=eoLR.phaseIndex;
     ...
     */
    avoidImmediateLeftRecursor(grammer) {
        if (this.isImmediateLeftRecursor(grammer)) {
            ...
            this.textOutput += notconts.substr(0, notconts.length - 1) + "\n";
            this.textOutput += conts + "ε" + "\n"; // 最后一个是空 ε代表是空

        } else {
            this.textOutput += grammer + "\n";
        }
    }
    ...
    // avoid(){
    //     var userinputs = this.textInput.split("\n");//把用户的输入按照行分开
    //     for (var ii = 0; ii < userinputs.length; ii++) {
    //         var headii = userinputs[ii].split("->")[0];
    //         var tailii = userinputs[ii].split("->")[1];
    //         for (var j = 0; j < ii; j++) {//把j带入ii中
    //             tailii = this.changeRules(userinputs, ii, j);
    //
    //         }
    //         userinputs[ii] = headii + "->" + tailii;//将能带入的式子带入后重新构建用户的输入来消除立即左递归
    //         this.avoidImmediateLeftRecursor(userinputs[ii]);//消除立即左递归
    //
    //     }
    //
    // }
     ...
     * @param grammer
     */
    isImmediateLeftRecursor(grammer) {
    ...
    if (this.phaseIndex == 3) {
        if (this.avoidLeftRecursorVeriable.j < this.avoidLeftRecursorVeriable.ii) {
            ...
            if (this.avoidLeftRecursorVeriable.ii < 3) {
        ...
        var userinputs = this.textInput.split("\n");//把用户的输入按照行分开
        var headii = userinputs[this.avoidLeftRecursorVeriable.ii - 1].split("->")[0];
        var tailii = userinputs[this.avoidLeftRecursorVeriable.ii - 1].split("->")[1];


        for (var j = 0; j < this.avoidLeftRecursorVeriable.ii - 1; j++) {


            tailii = this.changeRules(userinputs, this.avoidLeftRecursorVeriable.ii - 1, j);

        }
        userinputs[this.avoidLeftRecursorVeriable.ii - 1] = headii + "->" + tailii;//将能带入的式子带入后重新构建用户的输入来消除立即左递归

        this.avoidImmediateLeftRecursor(userinputs[this.avoidLeftRecursorVeriable.ii - 1]);//消除ii立即左递归
    ...
    class filechangeDirective{
        ...
        link(scope,element){
            element.on('change', this.$interval(() => this.onChange(), 1000));
            scope.$on('destroy', function () {
                element.off('change', this.$interval(() => this.onChange(), 1000));
            });
         this.$interval(() => this.onChange(), 1000);
        }
    ...
    angular
        ...
        // .directive('fileDropzone', () => new filechangeDirective)
...
function HomeController($scope) {
    ...
    function beautifyProduction(userinputs) {
        var firstHead = userinputs[0].split("->")[0];
    ...
    function compute_first(beta, first) {
        ...
        if (beta.length == 0 || beta[0] == "ε") {
            ...
            if (rnum == beta.length && firse.indexOf("ε") > -1) {
                if (result.indexOf("ε") == -1) {
                    result.push("ε");
                }
            } else {
                if (result.indexOf("ε") > -1) {
                    result.splice(result.indexOf("ε"), 1);
                }
            }
        }
    ...
    function findFollow(userinput, first) {
        ...
        follow_set[userinput[0].split("->")[0]].push("ε");
                    ...
                    for (var jnum = 0; jnum < btemp.length; jnum++) {
                        ...
                        if (isNonterminal(B) && B != '\'') {//如果是非终结符的话
                            ...
                            if (rfirst.indexOf("ε") == -1) {
                                ...
                                rfirst.splice(rfirst.indexOf("ε"), 1);
                                for (var rr = 0; rr < rfirst.length; rr++) {
                                    if (Barr.indexOf(rfirst[rr]) == -1) {
                                        Barr.push(rfirst[rr]);
                                    }
        ...
        var keyset = Object.keys(follow_set);
        for (var kn = 0; kn < keyset.length; kn++) {
            var valueset = follow_set[keyset[kn]];
    ...
    home.findFirst = function () {
        ...
        var keyset = Object.keys(first);
        for (var kn = 0; kn < keyset.length; kn++) {
            var valueset = first[keyset[kn]];
            home.firstinput += keyset[kn] + ":{";
            for (var vn = 0; vn < valueset.length; vn++) {
                home.firstinput += valueset[vn] + ",";
            }
